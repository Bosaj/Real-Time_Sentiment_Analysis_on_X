...
def map_prediction_to_sentiment(prediction):
    try:
        ...
        return "Unknown"
    sentiments = {0: "Negative", 1: "Positive", 2: "Neutral", 3: "Irrelevant"}
    ...
    try:
        ...
        for line in lines:
            if line.startswith("GROQ_API_KEY=") and "groq_key" in APP_CONFIG:
                ...
                uri_val = (
                    APP_CONFIG["cloud_uri"]
                    if APP_CONFIG["db_type"] == "cloud"
                    else "mongodb://localhost:27017/?directConnection=true"
                )
...
def validation():
    ...
    collection = db["TweetsPredictions"]
    predictions = list(
        collection.find({}, {"_id": 0, "content": 1, "prediction": 1, "confidence": 1}).sort("_id", -1).limit(50)
    )
...
def produce_tweets():
    ...
    if producer:
        message = {"content": tweet_content, "created_at": time.time()}
        if tracking_id:
    ...
    return jsonify({"status": "queued", "tweetContent": tweet_content, "tracking_id": tracking_id})
